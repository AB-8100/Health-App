...
//
// PURPOSE: The first-run multi-step setup wizard shown to new users.
//          Collects a user profile across 6–7 steps and hands it back to
//          App() via onComplete(profile) when the final step is confirmed.
//
// STEP FLOW:
//   welcome → basics → [cycle] → goal → imports → split → done
//   The [cycle] step is conditionally removed if:
//     - User selects sex: 'male' during 'basics'
//     - User explicitly opts out of cycle tracking
//   Step order is controlled by the `steps` array; `step` is just an index into it.
//
// PROFILE OBJECT (built up across steps, passed to onComplete on finish):
//   name        — display name
//   age         — age in years
//   sex         — 'male' | 'female' | ''
//   tracksCycle — bool | null (null until the cycle step is answered)
//   height      — cm
//   weight      — kg
//   goal        — 'strength' | 'muscle' | 'fat-loss' | 'active' | 'flexibility'
//   connected   — array of connected service IDs (e.g. ['strava', 'oura'])
//   splitDays   — 1–5 (training days per week)
//
// WHAT App() DOES WITH THE PROFILE:
//   Sets global profile state → affects all screens that use profile props
//   Sets plan.splitDays → determines which SPLIT template the Gym screens use
//   Sets onboardingActive = false → shows the main app
//
...
// ─────────────────────────────────────────────────────────────────────────────
// [DATA] IMPORT_SOURCES — services the user can connect during onboarding.
// cycleOnly: true means this source only appears if tracksCycle is true
// (Flo and Clue are period tracker apps — irrelevant for users not tracking cycle).
// In production, tapping a source would trigger OAuth for that service.
// Available data sources for import. Period tracker only shown if user tracks cycle.
...
// [DATA] GOAL_OPTIONS — the five training goals the user can select.
// The chosen goal affects coaching copy and the suggested split on the next screen.
const GOAL_OPTIONS = [
...
// ─────────────────────────────────────────────────────────────────────────────
// SECTION 2: OnboardingScreen COMPONENT
// ─────────────────────────────────────────────────────────────────────────────
function OnboardingScreen({ width = 390, height = 820, theme = 'light', onComplete, initial }) {
  const t = themes[theme];
  // [STATE] step — index into the `steps` array. Increments on 'Continue'.
  const [step, setStep] = React.useState(0);

  // [STATE] profile — accumulates across all steps.
  // Each step uses setProfile(p => ({ ...p, key: value })) to add its fields.
  ...
  // [LOGIC] Step array construction — the cycle step is conditionally excluded.
  // `steps` is derived on every render so it reacts to profile.sex changes immediately.
  ...
  // [DERIVED] progress — drives the progress bar fill (0–1).
...
// ─────────────────────────────────────────────────────────────────────────────
// EXPORT
// App() checks onboardingActive state — if true, renders OnboardingScreen
// instead of the normal screen router. onComplete() clears that flag.
// ─────────────────────────────────────────────────────────────────────────────
