// screen-food.jsx
// Full food diary + weekly calorie tracker.
//
// Primary view: today's food diary organised by meal (Breakfast / Lunch / Dinner / Snacks).
// Each meal section shows logged items with macros, a total, and a "+" add button.
// Adding food: search a built-in database of ~60 common foods (with protein/carbs/fat),
// enter a portion, and it calculates everything. Custom entry also available.
// Top of screen: calorie ring + macro bars for today. Swipe left/right or use arrows
// to browse other days.
// Bottom card: weekly calorie bars (kept from previous version).
//
// Data model (entries now carry full macros):
//   foodLog['YYYY-MM-DD'].entries = [
//     { id, name, calories, protein, carbs, fat, meal, time, grams }
//   ]
...
function foodDateKey(date) { return date.toISOString().split('T')[0]; }
...
function FoodScreen({
  ...
  // Day target (adjusting for gym/activity)
  const vDow     = foodDow(viewDate);
  const split    = window.SPLITS && plan.splitDays ? window.SPLITS[plan.splitDays] : null;
  ...
  const dayTarget = base + ((isGymDay || completedGymThisDay) ? gymBoost : 0) + (hasActivity ? 120 : 0);
  ...
  const weekDays  = Array.from({length:7}, (_,i) => {
    ...
    const dow = i;
    const isGym = split && split.schedule[dow] !== '—';
    const compGym = completedSessions.some(s => foodDateKey(new Date(s.date)) === foodDateKey(d));
    const hasAct = (activities[dow] || []).some(a => a.duration >= 30);
    const tgt = base + ((isGym || compGym) ? gymBoost : 0) + (hasAct ? 120 : 0);
