...
//
//   ExerciseImage        — a shared placeholder component used as an exercise
//                          "thumbnail" across all gym screens. Renders a
//                          colour-gradient card since there are no real photos.
//                          Exported to window.ExerciseImage.
//
//   ExerciseLibraryScreen — a full searchable/filterable exercise reference.
//                           Pulls exercises from window.EX_LIB (defined in
//                           screen-gym-plan.jsx, which must load first).
//
// DEPENDENCY: screen-gym-plan.jsx must be loaded before this file so that
//             window.EX_LIB is available.
//
// EXPORTS: window.ExerciseImage, window.ExerciseLibraryScreen
// ─────────────────────────────────────────────────────────────────────────────

// [FILE HEADER — see top of file for full description]

// ─────────────────────────────────────────────────────────────────────────────
// SECTION 1: ExerciseImage COMPONENT
// A coloured gradient card used wherever exercise thumbnails appear.
// There are no real photos — this is the designed placeholder.
//
// COLOUR LOGIC:
//   Type (compound/accessory/core/mobility) maps to a colour family (rust/amber/violet/moss).
//   Within each family, 5 palette variants exist. A simple hash of the exercise ID
//   picks which variant — so the same exercise always gets the same card.
//   The gradient angle also varies by hash for visual rhythm.
//
// In production: replace the gradient card with a real <img> or <video> element.
// The component API (exerciseId, size, radius) stays the same.
// ─────────────────────────────────────────────────────────────────────────────
// Type → color pair used for exercise placeholder cards.
...
// ─────────────────────────────────────────────────────────────────────────────
// SECTION 2: ExerciseLibraryScreen
// A full searchable exercise reference, accessed from the Gym Hub via 'Browse'.
//
// FILTERS:
//   Type chips    — All / Compound / Accessory / Core / Mobility
//   Muscle chips  — Chest / Back / Shoulders / Arms / Legs / Core / Mobility
//   Text search   — matches exercise name or muscle group (case-insensitive)
// All three filters compose: a result must pass all active filters.
// Filtering runs client-side via React.useMemo — no network call.
//
// MUSCLE_GROUPS maps UI filter IDs to the muscle strings used in EX_LIB,
// since EX_LIB uses specific strings ('Lats/Biceps', 'Upper back') but the
// filter chip just says 'Back'.
//
// DETAIL MODAL: tapping an exercise shows a bottom sheet with name, muscle,
// type badge, and the ExerciseImage card.
// ─────────────────────────────────────────────────────────────────────────────
...
function ExerciseLibraryScreen({ width = 390, height = 820, theme = 'light',
  ...
  // [DERIVED] filtered — the list of exercises matching all active filters.
  // Recomputes only when search, muscleFilter, or typeFilter changes.
  ...
  // [DERIVED] muscleCounts — exercise count per muscle group.
  // Used to show numbers on the muscle filter chips (e.g. 'Chest · 5').
...
Object.assign(window, {
  ...
  ACTIVITY_TYPES, MUSCLE_GROUPS, EX_TYPE_COLORS
});
